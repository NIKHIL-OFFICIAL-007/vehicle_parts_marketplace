...
<body class="bg-gray-50 text-gray-900">
  ...
  <section id="home" class="hero-bg text-white py-32 px-6 text-center">
    <div class="container mx-auto">
      ...
      <div class="flex flex-col sm:flex-row justify-center gap-5 mt-8">
        <!-- Browse Parts -->
        <?php if ($logged_in): ?>
          <a href="buyer/dashboard.php" class="btn-primary px-8 py-4 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition flex items-center justify-center">
            <i class="fas fa-search mr-2"></i> Browse Parts
          </a>
        <?php else: ?>
          <a href="#" onclick="showLoginAlert(); return false;" class="btn-primary px-8 py-4 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition cursor-pointer flex items-center justify-center">
            <i class="fas fa-search mr-2"></i> Browse Parts
          </a>
        <?php endif; ?>
        ...
        <?php if ($logged_in): ?>
          <?php if ($role === 'seller' && $role_status === 'approved'): ?>
            <a href="seller/dashboard.php" class="btn-outline px-8 py-4 text-lg font-semibold rounded-xl bg-white bg-opacity-20 backdrop-blur-sm hover:bg-opacity-30 transition flex items-center justify-center">
          ...
          <?php else: ?>
            <button onclick="showRoleAlert('You need to be an approved seller to sell parts.')" 
                    class="btn-outline px-8 py-4 text-lg font-semibold rounded-xl bg-white bg-opacity-20 backdrop-blur-sm hover:bg-opacity-30 transition flex items-center justify-center cursor-pointer">
    ...
    <div class="container mx-auto px-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <?php foreach ([
          ...
          <div class="bg-gray-50 p-8 rounded-2xl shadow-md hover:shadow-xl transition role-card">
    ...
    <div class="container mx-auto px-6 text-center">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <?php foreach (['Create Account', 'Browse or List', 'Order & Pay', 'Track & Review'] as $i => $title): ?>
          <div class="relative">
            <div class="w-16 h-16 bg-primary text-white rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4"><?= $i+1 ?></div>
  ...
  <section id="role-apply" class="py-20 bg-secondary text-white">
    <div class="container mx-auto px-6">
      <h2 class="text-3xl font-bold text-center mb-12">Apply Roles</h2>
  ...
  <!-- Login Alert -->
  <div id="loginAlert" class="fixed top-4 right-4 hidden p-4 bg-red-500 text-white rounded-lg shadow-lg max-w-xs z-50">
  ...
  <!-- Role Alert -->
  <div id="roleAlert" class="fixed top-4 right-4 hidden p-4 bg-yellow-500 text-white rounded-lg shadow-lg max-w-xs z-50">
    <div class="flex items-center">
